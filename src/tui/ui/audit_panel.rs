...
pub fn render(f: &mut Frame, area: Rect) {
    ...
    header_spans.push(Span::styled("Audit Trail", Style::default().fg(Color::Rgb(255, 145, 115)).add_modifier(Modifier::BOLD)));
    ...
    // Audit log table
    let header_cells = ["Timestamp", "User", "Action", "Resource", "Result", "Details"]
        .iter()
        .map(|h| {
            Cell::from(*h).style(
                Style::default()
    ...
    let entries = vec![
        ("14:32:05", "admin", "CREATE", "VM/web-server-03", "Success", "From template ubuntu-22"),
        ("14:28:12", "admin", "START", "VM/web-server-01", "Success", "Cold boot"),
        ("14:15:44", "ci-bot", "SNAPSHOT", "VM/db-primary", "Success", "Auto backup"),
        ("13:58:21", "ops-user", "MIGRATE", "VM/worker-01", "Success", "node-1 -> node-3"),
        ("13:45:03", "admin", "DELETE", "Snapshot/old-snap-12", "Success", "Cleanup"),
        ("13:30:18", "dev-user", "STOP", "VM/test-vm-alpha", "Success", "Graceful shutdown"),
        ("13:12:55", "ci-bot", "SCALE", "VMPool/workers", "Success", "Replicas 3 -> 5"),
        ("12:58:30", "ops-user", "UPDATE", "VM/api-gateway", "Failed", "Insufficient resources"),
        ("12:45:11", "admin", "RBAC", "Role/vm-viewer", "Success", "Added read permissions"),
        ("12:30:02", "system", "ALERT", "Node/node-4", "Warning", "Unreachable"),
    ];

    let rows = entries.iter().map(|(ts, user, action, resource, result, details)| {
        ...
        Row::new(vec![
            ...
            Cell::from(*action).style(Style::default().fg(action_color).add_modifier(Modifier::BOLD)),
            Cell::from(*resource),
