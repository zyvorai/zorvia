...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ActionType {
    StartVm, StopVm, RestartVm, DeleteVm, CreateSnapshot, RestoreSnapshot,
    MigrateVm, ScaleResources, NavigateView, ExecuteCommand, Wait,
}
...
pub enum PlaybackStatus { Running, Paused, Completed, Failed }

impl MacroManager {
    pub fn new() -> Self {
        Self { macros: Vec::new(), recording: None }
    }

    pub fn start_recording(&mut self, name: &str) {
        self.recording = Some(MacroRecording { name: name.to_string(), started_at: Utc::now(), actions: Vec::new() });
    }
    ...
    pub fn stop_recording(&mut self, description: &str) -> Option<String> {
        if let Some(recording) = self.recording.take() {
            ...
            let m = Macro {
                id: id.clone(), name: recording.name, description: description.to_string(),
                actions: recording.actions, created_at: Utc::now(),
                last_run: None, run_count: 0, tags: Vec::new(),
            };
            self.macros.push(m);
            Some(id)
        } else { None }
    ...
    pub fn cancel_recording(&mut self) { self.recording = None; }
    ...
    pub fn play(&mut self, id: &str) -> Option<PlaybackState> {
        if let Some(m) = self.macros.iter_mut().find(|m| m.id == id) {
            ...
            Some(PlaybackState {
                macro_id: id.to_string(), current_step: 0, total_steps: m.actions.len(),
                status: PlaybackStatus::Running, started_at: Utc::now(), errors: Vec::new(),
            })
        } else { None }
    }

    pub fn list(&self) -> &[Macro] { &self.macros }
}
