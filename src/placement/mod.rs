...
impl PlacementEngine {
    ...
    pub fn add_node(&mut self, node: PlacementNode) {
        self.nodes.push(node);
    }
    pub fn add_constraint(&mut self, constraint: PlacementConstraint) {
    ...
    pub fn find_placement(
        ...
        for (i, node) in self.nodes.iter().enumerate() {
            if node.available_cpu < cpu_req
                || node.available_memory_gb < memory_req
                || node.available_storage_gb < storage_req
            {
                continue;
            }
            if node.current_vm_count >= node.max_vms {
                continue;
            }
        ...
        candidates.first().map(|&(idx, score)| {
            ...
            PlacementDecision {
                ...
                score,
                reason: format!(
                    "Best fit: {:.1} CPU, {:.1}Gi memory available",
                    node.available_cpu, node.available_memory_gb
                ),
                alternatives: candidates
                    .iter()
                    .skip(1)
                    .take(3)
                    .filter_map(|&(i, s)| self.nodes.get(i).map(|n| (n.name.clone(), s)))
                    .collect(),
            }
        })
    }
    ...
    fn score_node(&self, node: &PlacementNode, cpu_req: f64, memory_req: f64) -> f64 {
        match self.config.strategy {
            PlacementStrategy::LeastLoaded => {
                ...
                let mem_free_ratio =
                    node.available_memory_gb / (node.available_memory_gb + memory_req);
            ...
            PlacementStrategy::BinPacking => {
                ...
                let mem_util =
                    1.0 - (node.available_memory_gb / (node.available_memory_gb + memory_req));
                (cpu_util + mem_util) / 2.0
            }
            PlacementStrategy::Spread => 1.0 / (node.current_vm_count as f64 + 1.0),
    ...
    pub fn suggest_rebalance(&self) -> Vec<RebalanceSuggestion> {
        ...
        let avg_load: f64 = self
            .nodes
            .iter()
            .map(|n| n.current_vm_count as f64)
            .sum::<f64>()
        ...
        for node in &self.nodes {
            if node.current_vm_count as f64 > avg_load * (1.0 + self.config.rebalance_threshold) {
                if let Some(target) = self.nodes.iter().find(|n| {
                    n.name != node.name
                        && (n.current_vm_count as f64)
                            < avg_load * (1.0 - self.config.rebalance_threshold)
                }) {
                    suggestions.push(RebalanceSuggestion {
                        ...
                        reason: format!(
                            "Node {} has {} VMs (avg: {:.1})",
                            node.name, node.current_vm_count, avg_load
                        ),
                    });
                }
            }
