...
impl SecurityGroup {
    pub fn new(name: impl Into<String>) -> Self {
        let name_str = name.into();
        let id = format!("sg-{}-{}", name_str.to_lowercase().replace(' ', "-"), Utc::now().timestamp());
...
impl PolicyManager {
    ...
    pub fn high_priority_rules(&self) -> Vec<&NetworkPolicyRule> {
        self.rules.values().filter(|r| r.is_high_priority()).collect()
    }
    ...
    pub fn enforcing_policies(&self) -> Vec<&NetworkPolicy> {
        self.policies.values().filter(|p| p.is_enforcing()).collect()
    }
}
...
mod tests {
    ...
    fn test_network_policy_rule() {
        let rule = NetworkPolicyRule::new("allow-http", PolicyAction::Allow, TrafficDirection::Ingress);
    ...
    fn test_rule_with_priority() {
        let rule = NetworkPolicyRule::new("high-prio", PolicyAction::Deny, TrafficDirection::Egress)
            .with_priority(5);
    ...
    fn test_rule_add_source_cidr() {
        let mut rule = NetworkPolicyRule::new("test", PolicyAction::Allow, TrafficDirection::Ingress);
    ...
    fn test_rule_add_destination_cidr() {
        let mut rule = NetworkPolicyRule::new("test", PolicyAction::Allow, TrafficDirection::Egress);
    ...
    fn test_policy_with_enforcement_mode() {
        let policy = NetworkPolicy::new("test", "default")
            .with_enforcement_mode(EnforcementMode::Audit);
    ...
    fn test_policy_is_enforcing() {
        ...
        let policy2 = NetworkPolicy::new("test2", "default")
            .with_enforcement_mode(EnforcementMode::Audit);
    ...
    fn test_sg_with_description() {
        let sg = SecurityGroup::new("test-sg")
            .with_description("Test security group");
    ...
    fn test_manager_rules_by_action() {
        ...
        manager.add_rule(NetworkPolicyRule::new("r1", PolicyAction::Allow, TrafficDirection::Ingress));
        manager.add_rule(NetworkPolicyRule::new("r2", PolicyAction::Deny, TrafficDirection::Egress));
        manager.add_rule(NetworkPolicyRule::new("r3", PolicyAction::Allow, TrafficDirection::Both));
