...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum RiskType { DataLoss, Downtime, Performance, Network, Storage, Compatibility }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum RiskSeverity { Critical, High, Medium, Low }

pub fn simulate_migration(vm_name: &str, source: &str, target: &str, memory_gb: f64, cpu_cores: f64) -> DryRunResult {
    ...
    if memory_gb > 32.0 {
        risks.push(MigrationRisk {
            risk_type: RiskType::Downtime, severity: RiskSeverity::Medium,
    ...
    DryRunResult {
        vm_name: vm_name.to_string(), source_node: source.to_string(), target_node: target.to_string(),
        estimated_duration_secs: duration, estimated_downtime_secs: downtime, data_transfer_gb: data_transfer,
        resource_impact: ResourceImpact {
            source_cpu_freed: cpu_cores, source_memory_freed_gb: memory_gb,
